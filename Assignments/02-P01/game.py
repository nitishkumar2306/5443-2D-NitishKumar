#  __          ______  _____  _____  _      ______   _______     _______          __  __ ______ 
#  \ \        / / __ \|  __ \|  __ \| |    |  ____| |  __ \ \   / / ____|   /\   |  \/  |  ____|
#   \ \  /\  / / |  | | |__) | |  | | |    | |__    | |__) \ \_/ / |  __   /  \  | \  / | |__   
#    \ \/  \/ /| |  | |  _  /| |  | | |    |  __|   |  ___/ \   /| | |_ | / /\ \ | |\/| |  __|  
#     \  /\  / | |__| | | \ \| |__| | |____| |____  | |      | | | |__| |/ ____ \| |  | | |____ 
#      \/  \/   \____/|_|  \_\_____/|______|______| |_|      |_|  \_____/_/    \_\_|  |_|______|
# ------------------------------------------------------------------------------------------------
...
# importing words from the csv file
filename="words.csv"
...
# randomly generating a number in the given range
num = random.randint(1,140)
print(guessWord[num])

# loading the image into the grid
grid = [
...
# Highlight the selected box in the grid
def draw_box():
    for i in range(2):
...
# To get the co-ordinated of the of the mouse click
def get_cord(pos):
    global x
    global y
    y = pos[1]//dif 
...
# Fill the value entered into the cell
def draw_val(data):
    x_cord = int(x)
    y_cord = int(y)
...
while run:    
    ...
    # checking If mouse clicked on grid
    if (x_cord >=2 and x_cord <=7) and ((y_cord >=2 and y_cord <=7)):
        ...
        if value != 0:
            if (y_cord-2) >= var_j_val+1:
                ...
                flag1 = 0
                value = 0
# validate If value entered is matching with the word
    if flag2 == 1:
        for j in range(0,6):
            x_val = var_j_val
